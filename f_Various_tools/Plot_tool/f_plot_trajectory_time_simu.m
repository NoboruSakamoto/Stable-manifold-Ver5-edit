function []=f_plot_trajectory_time_simu(txpdata,pltobject,PLToption,colorset)
% ========================= f_plot_trajectory_time_simu =============================
% グラフプロットで利用(時系列プロット)
% 与えられた解軌道データ，Lineオブジェクト，プロット設定を読み込んで
% 時系列のグラフプロットを行う
% シミュレーション結果プロット用にカラーコードを読み込んでプロットカラーを指定可能
% 作成者　    :2021/3/1 竹田 賢矢
...
for i_plot=1:Plf
    ...
    if i_plot>3
        ...
        plt_T(i_plot).Color = colorset(i_plot,:);
        plt_T(i_plot).MarkerFaceColor=colorset(i_plot,:);
        plt_T(i_plot).MarkerEdgeColor=colorset(i_plot,:);
    end
